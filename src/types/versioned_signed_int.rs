...
impl<F: FlagTrait> Mp4VersionedWritable<F> for VersionedSignedU32 {
    ...
    fn versioned_write<W: WriteMp4>(&self, version: u8, _: F, writer: &mut W) -> Result<usize, MP4Error> {
        Ok(match version {
            0 => match self {
                VersionedSignedU32::Unsigned(it) => it.write(writer)?,
                VersionedSignedU32::Signed(it) =>  {
                    if *it < 0 {
                        ...
                        (*it as u32).write(writer)?
                    }
                }
            },
            _ => match self {
                VersionedSignedU32::Unsigned(it) => (*it as i32).write(writer)?,
                VersionedSignedU32::Signed(it) => it.write(writer)?
            }
        })
    }
}
